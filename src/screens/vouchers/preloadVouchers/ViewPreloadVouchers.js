...
function ViewPreloadVouchers() {
  ...
  const initPageModule = async () => {
    try {
      ...
      const ep = endpoint.getDataPreloadVoucherSchemaEndpoint(`view/${id}`);
      console.log(ep, 'ep');
      ...
      const memberRes = await Axios.get(
        endpoint.getModuleDataEndpoint(`bb_loyal2_vouchers_precodes/${id}`)
      );
  ...
  const onDelete = async id => {
    try {
      ...
      _isMounted.current && setLoadingSchema(false);
      navigate('/datamanager/bb_loyal2_vouchers_precodes/list');
    }
  };
  return (
    <>
      ...
      <Row className="mx-4" align="middle">
        <Col xs={23} sm={23} md={12} lg={12} xl={12} xxl={12}>
          <Title level={4}>Viewing pre-loaded voucher code</Title>
        </Col>
        <Col
          xs={23}
          sm={23}
...
export default ViewPreloadVouchers;
