...
function UpdateAllVouchers() {
  ...
  const initPageModule = async () => {
    try {
      ...
      const ep = endpoint.getDataAllVoucherSchemaEndpoint(`${routeKey}/${id}`);
      ...
      set_available_for_self_selectionYN(
        Number(layoutSchema.data[0][0].available_for_self_selectionYN)
      );
...
export default UpdateAllVouchers;
