...
function ListScanReason() {
  ...
  const initPageModule = async () => {
    try {
      ...
      const ep = endpoint.getDataQuickScanReasonSchemaEndpoint('list');
  ...
  const editRow = row => {
    navigate('/datamanager/bb_loyal2_quickscan_reasons/edit/' + row._id);
  };
  const deleteRow = () => {
    _array.length > 0
  ...
  const row_select = row => {
    navigate('/datamanager/bb_loyal2_quickscan_reasons/view/' + row._id);
  };
  ...
  return (
    <>
      <Row className="mx-4 mt-6">
        <Col>
          <Title level={4}>All QuickScan reasons</Title>
        </Col>
      </Row>
